...
\paragraph*{Rule 5 (algorithmic only).}
...
\forall i,\ 1 \le i \le n,\ \Gamma \vdash C_i : \Delta_i \rightarrow T\:\Delta_i'^* \\
\forall i,\ 1 \le i \le n,\ \Gamma \vdash N_{i} : \Delta_{i} \rightarrow Q%
}{%
...
Let $T$ be an inductive family with constructor set $C$, and let $S \subseteq C$.
Define $D : T \to Type$ by inspecting only the \emph{head constructor} of
...
\[
  f : \{T :: S\} \to \Sigma(x:T), D(x).
\]
Moreover, in any host theory that validates refinement introduction
(from $x:T$ and $D(x)$ to $x:\{T::S\}$), this map is part of an
isomorphism $\{T::S\} \simeq \Sigma(x:T), D(x)$.
\end{theorem}
\begin{proof}[Proof sketch]
Define $f(x) \coloneqq (x, \star)$.
To justify $\star : D(x)$, use the canonical-forms property for signature
types: if $x : \{T::S\}$ is a value, its canonical form is
$C\,\overrightarrow{u}$ with $C \in S$, hence $D(C\,\overrightarrow{u}) \equiv \top$.

For the converse direction, assume refinement introduction:
from $(y,d) : \Sigma(x:T), D(x)$ we obtain $y : \{T::S\}$ and define
...
Our implementation now has two components. The backend encoding in Lambdapi (\texttt{encode.lp}, 666 lines) defines signatures, constructor-list well-formedness, subtyping checks, and signature-directed elimination on top of the $\lambda \Pi$ calculus modulo rewriting~\cite{Saillard}. The frontend transpiler (\texttt{Main.hs}, 956 lines) parses a restricted surface syntax and lowers it to backend terms. Both components are in \texttt{subindex/subtyping/}. The lowering mirrors Rules~1--7 from Section~\ref{sec:rules}. Signature annotations become \texttt{signature\_term}. Constructor introductions become \texttt{signature\_intro}. Signature-only matches become \texttt{signature\_case}. Appendix~\ref{app:transpiler} shows representative input and generated output.
...
\end{lstlisting}
\caption{Mini-language rendering of the non-empty vector example}
